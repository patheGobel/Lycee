...
Si \( f \) est continue en \(x_{0}\) et \( g \) continue en \( f(x_{0}) \) alors \( gof \) est continue en \(x_{0}\) donc la composition de deux fonctions continues est une fonction continue.
...
Le tableau ci-dessous précise \( f(K) \) suivant la nature de \( K \) et du sens de variation de \( f \).
\end{itemize}
...
\hline
]a;b[		& \( ]\lim\limits_{x\to b^{+}}f(x);\lim\limits_{x\to b^{-}}f(x)[ \) &	\( ]\lim\limits_{x\to b^{-}}f(x);\lim_{x\to b^{+}}f(x)[ \)	\\
\hline
...
Si la fonction \( f \) est \textcolor{red}{ continue }  sur \( [a ; b] \) et si le réel \( m \) est compris entre \( f(a) \) et \( f(b) \), alors l'équation \( f(x) = m \) admet \textcolor{red}{ au moins } une solution dans \( [a ; b] \).
...
Soit la fonction \( f(x)=\frac{1}{2}x^{3}+x-5 \)

Montrer que l'équation \( f(x) = 5 \) admet une solution sur \( [-2 ; 4] \).
...
\begin{itemize}
\item Si \(  m = 0 \) il suffit  de montrer que \( \lim_{x \to a} f(x) \times \lim_{x \to b} f(x) <0 \)
\item Il faut toujours commencer par montrer que \( f \) est solution.
\end{itemize}
...
Montrer que l’équation \( f(x) = 0 \) admet au moins une solution dans \( [-2 ; -1] \).

\textbf{\underline{\correction}}

\item \textbf{\textcolor{blue}{\underline{Théorème d’existence et d’unicité d’une solution}}}

Si la fonction \( f \) est \textcolor{red}{ continue } et \textcolor{red}{ strictement monotone } (croissante ou bien décroissante) sur \( [a ; b] \). Pour tout réel \( m \) compris entre \( f(a) \) et \( f(b) \), alors l'équation \( f(x) = m \) admet une \textcolor{red}{unique solution }\(\alpha\in [a ; b] \).
...
\item \textbf{\textcolor{blue}{\underline{Encadrement de la racine \( \alpha \) à \( \epsilon \) près}}}
      ...
      Si \( f \) fonction \textcolor{red}{contenu} \textcolor{red}{strictement} sur un intervalle \( [a;b] \), telle que \( f(a)\geq 0 \) et \( f(b) \leq 0\); alors il existe au moins un réel \( \alpha \) dans  l'intervalle \( [a;b] \) tel que \( f(\alpha) = 0 \).
...
\begin{enumerate}
    ...
    \item Déterminer l'expression explicite de la fonction réciproque \( f^{-1} \) de la fonction \( f \) ?
...
\hline
\textbf{Limite} & \textbf{Dérivabilité en \( x_0 \)} & \textbf{Interprétation géométrique : \( (C_f) \) admet} \\
\hline
...
\begin{figure}[H]% Forcer l'image à cet endroit
\centering
\includegraphics[width=0.5\textwidth]{axeSymetrie.png}
