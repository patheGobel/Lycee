...
\textcolor{red}{ $f$ n'est pas dérivable à droite de $0$ et on a une démi-tangent orientée vers le bas en $0^+$  }
...
\begin{tikzpicture}
   \tkzTabInit[lgt = 2.5, espcl = 3, deltacl = 0.7]{$x$ / 1 , $-\dfrac{1}{x-1}$ / 1 }{$-\infty$, $1$, $+\infty$}
   \tkzTabLine{,+,d,h,}
\end{tikzpicture}
...
\end{itemize}
    ...
    \begin{tikzpicture}[node style/.style={fill opacity=0,text opacity=1}]
        \tkzTabInit[espcl=1.75]{$x$/.5,$f'(x)$/.7,$f$/1.5}{$-\infty$,$0$,$+\infty$}
        \tkzTabLine{,+,d,+}
