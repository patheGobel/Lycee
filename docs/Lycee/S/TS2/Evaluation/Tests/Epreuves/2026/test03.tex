...
\begin{center}
    \begin{tabular}{@{} p{5cm} p{5cm} p{5cm} @{}}\\[0.2cm] % 3 colonnes avec largeurs fixées
        Lycée Dindéfélo & Test 3 & 10 Novembre 2025 \\
    \end{tabular}
...
\question
\[\text{ Pour calculer la limite \( \lim_{x \to a} h(g(x)) \), il faut d'abord déterminer \( \lim_{x \to a} g(x) = \dots \) puis, en utilisant cette}\]

\[\text{  valeur, calculer \( \lim_{y \to \dots} h(y) = \dots \).}\]
