...
\begin{enumerate}
    ...
    \[ p_1 = P(A_1) = P(A_1 \mid A_0) \times P(A_0) = 0,6 \times 1 = \mathbf{0,6} \]

    \item L'arbre pondéré reliant le jour $n$ au jour $n+1$ est le suivant :
    \begin{center}
    \usepackage{istgame} % À ajouter dans votre préambule si ce n'est pas fait
    \begin{istgame}[scale=1.2]
        \xtdistance{1.5cm}{3.5cm}
        \istroot(0)
            \istb{p_n}[al]{A_n}
            \istb{1-p_n}[bl]{M_n}
            \endgroup
        \xtdistance{1cm}{2.5cm}
        \istroot(1)(0-1)
            \istb{0,6}[al]{A_{n+1}}
            \istb{0,4}[bl]{M_{n+1}}
            \endgroup
