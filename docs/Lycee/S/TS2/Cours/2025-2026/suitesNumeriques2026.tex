...
plot ({\x},{2*sqrt(\x)}) node[right] {$\mathcal{C}_g$};

% Bissectrice
\draw[red,thick] (0,0) -- (4.5,4.5) node[right] {$y=x$};

% Valeur initiale
\def\u{1.5} % Une valeur comme 1.5 donne un peu plus d'espace au départ

% Point initial sur l'axe
\filldraw[black] (\u,0) circle (1.5pt) node[below] {$u_0$};

% Construction des 3 premiers termes (u0 à u3)
\foreach \n in {1,2,3} {
...
\begin{cases} 
u_0 = 1 \\ 
u_{n+1} = \sqrt{5u_n} + 1 \quad \text{pour tout } n \in \mathbb{N}
\end{cases}
\)

\begin{enumerate}
    \item Calculer $u_1$, $u_2$ et $u_3$.
    \item Representer graphiquement les premiers termes de la suite $(u_n)$.
\end{enumerate}
...
\section*{\underline{\textbf{\textcolor{red}{VII. Étude complète d'une suite définie par récurrence}}}}
