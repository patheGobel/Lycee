...
% ==========================================
\usepackage[utf8]{inputenc} % Encodage des caractères d'entrée (inutile avec XeLaTeX/LuaLaTeX)
\usepackage[T1]{fontenc}   % Encodage des polices de sortie pour la gestion des accents
\usepackage{times}        % Police Times New Roman pour le texte
\usepackage{lmodern}      % Police Latin Modern améliorée
\usepackage{pifont}       % Symboles et caractères spéciaux (puces, coches, flèches)
...
\usepackage{multicol}     % Gestion de la mise en page sur plusieurs colonnes

% ==========================================
% 3. MATHÉMATIQUES ET SYSTÈMES
% ==========================================
\usepackage{amsmath}     % Environnements et commandes mathématiques de base
\usepackage{amsfonts}    % Polices et symboles mathématiques additionnels
\usepackage{amssymb}     % Symboles mathématiques avancés
\usepackage{mathrsfs}    % Police calligraphique mathématique (\mathscr)
\usepackage{systeme}     % Alignement et mise en forme automatique des systèmes d'équations

% Force l'affichage grand format (displaystyle) pour les formules intégrées au texte
\everymath{\displaystyle}

% ==========================================
% 4. COULEURS, TEXTE ET FORMATAGE
% ==========================================
\usepackage{color}       % Gestion des couleurs de base
\usepackage{soul}        % Gestion du soulignement et de la mise en valeur du texte
\usepackage{verbatim}    % Insertion de texte brut et blocs de commentaires

% Définition d'une couleur personnalisée
\definecolor{myorange}{rgb}{1.0, 0.8, 0.0}

% Commandes personnalisées pour le soulignement et l'espacement
...
% ==========================================
\usepackage{array}       % Amélioration du traitement des tableaux
\usepackage{multirow}    % Permet de fusionner des cellules sur plusieurs lignes
\usepackage{diagbox}     % Séparation diagonale de cellules de tableau
\usepackage{float}        % Contrôle avancé du placement des figures et tableaux
\usepackage{varwidth}    % Ajustement automatique de la largeur des boîtes

% ==========================================
% 6. LISTES ET PUCES PERSONNALISÉES
% ==========================================
\usepackage{enumitem}    % Personnalisation avancée des listes à puces et numérotées


% Styles TikZ pour puces circulaires et carrées
...
% Application aux niveaux 1 et 2 des listes numérotées
...
% ==========================================
\usepackage{tikz}        % Moteur de dessin vectoriel
\usetikzlibrary{patterns, arrows, arrows.meta, shapes.geometric, fit} 
% Extensions TikZ :
% - patterns : motifs de hachures et remplissages
% - arrows / arrows.meta : gestion et styles des flèches
% - shapes.geometric : formes géométriques complexes
% - fit : enveloppe et ajustement de nœuds

\usepackage{tkz-tab}     % Tableaux de variations et de signes mathématiques
\usepackage{istgame}     % Tracé d'arbres de décision et théorie des jeux

% ==========================================
% 8. BOÎTES ET ENCADRÉS (TCOLORBOX)
% ==========================================
\usepackage[most]{tcolorbox} % Boîtes encadrées stylisées
\tcbuselibrary{skins, hooks} % Extensions graphiques (skins) et accroches d'actions (hooks)

% Environnement de boîte personnalisée pour les exemples
\newtcolorbox{exa}[2][]{
  enhanced, breakable,
...
% ==========================================
\usepackage{eso-pic}     % Ajout d'éléments en arrière-plan des pages

% Filigrane "PGB" en arrière-plan au centre de chaque page
\AddToShipoutPicture{
  \AtTextCenter{%
...
% ==========================================
% 10. COMPTEURS ET COMMANDES DE COURS
% ==========================================
% Compteur et commande pour les exemples
\newcounter{exemple}
\newcommand{\exemple}{%
...
% Compteur et commande pour les exercices d'application
...
% Compteur et commande pour les corrections
...
% ==========================================
% Doit toujours être chargé en dernier pour éviter la rupture de macros d'autres paquets
...
L’objectif de ce cours est de donner un sens précis à la notion d’aire sous une courbe, puis de montrer comment la calculer efficacement. Pour cela, nous allons emprunter deux chemins complètement différents, mais qui aboutiront au même résultat.

Considérons la fonction constante \(f(x)=3\) sur l'intervalle \([1\,;5]\).

\subsubsection*{\underline{\textbf{\textcolor{red}{Première méthode : une approche géométrique}}}}

La courbe représentative de \(f\) est une droite horizontale. La région comprise entre cette courbe, l'axe des abscisses et les droites d'équations \(x=1\) et \(x=5\) est un rectangle.
...
Son aire est donc \(A=4\times3=12.\)
...
\begin{tikzpicture}[scale=0.8]
    ...
    % Fonction
    \draw[thick,blue] (0,3)--(6,3) node[right]{$y=f(x)=3$};
    ...
    % Aire
    \fill[blue!20] (1,0) rectangle (5,3);

    % Graduations
    \foreach \x in {1,5}
        \draw (\x,0.08)--(\x,-0.08) node[below]{\x};

    \draw (0.08,3)--(-0.08,3) node[left]{3};
\end{tikzpicture}
\end{center}

On obtient donc \(A=12.\)

\vspace{0.4cm}

\subsubsection*{\underline{\textbf{\textcolor{red}{ Deuxième méthode : une approche calculatoire }}}}

Une primitive de la fonction \(f\) est \( F(x)=3x. \)\vspace{0.4cm}
...
Nous avons donc calculé une même aire de deux façons différentes :

\begin{itemize}
    \item par une méthode géométrique ;
    \item par un simple calcul utilisant une primitive.
\end{itemize}

Cette égalité n'est pas un hasard. Elle reste vraie pour toute fonction continue.

\vspace{0.4cm}

Soit \(f\) une fonction continue sur un intervalle \(I\), \(F\) et \(G\) deux primitives de \(f\) sur \(I\).

Il existe une constante réelle \(c\) telle que \( G(x)=F(x)+c,\qquad \forall x\in I. \)\vspace{0.4cm}
...
En soustrayant les deux égalités, on obtient \( G(b)-G(a)=F(b)-F(a). \)

Ainsi, la quantité \( F(b)-F(a) \)

ne dépend pas de la primitive choisie.

Cette quantité est donc bien définie. On la note \( \boxed{\int_a^b f(x)\,dx=F(b)-F(a)} \)

et elle représente l'aire algébrique comprise entre la courbe représentative de \(f\), l'axe des abscisses et les droites d'équations \(x=a\) et \(x=b\).
...
\subsubsection*{\textbf{\textcolor{red}{Exemple 1}}}
...
\subsubsection*{\textbf{\textcolor{red}{Exemple 2}}}
...
\subsubsection*{\textbf{\textcolor{red}{Propriété 1 }}}
...
\subsubsection*{\underline{\textbf{\textcolor{red}{3. Intégrale et Primitive :}}}}
...
\subsubsection*{\textbf{\textcolor{red}{Remarque}}}
\begin{itemize}
...
\subsubsection*{Exercice d'application : Fonction changeant de signe}
...
\subsection*{\underline{\textbf{\textcolor{red}{Exemple d'application de la méthode ALPES}}}}

On souhaite calculer l'intégrale suivante à l'aide d'une intégration par parties :
\[ I = \int_{1}^{e} x \ln(x) \, dx \]

\subsubsection*{Étape 1 : Choix de $u(x)$ et $v'(x)$ avec ALPES}
L'expression à intégrer contient deux types de fonctions :
\begin{itemize}
    \item $x$ qui est une fonction \textbf{P}olynôme.
    \item $\ln(x)$ qui est une fonction \textbf{L}ogarithme.
\end{itemize}
Dans le mot A-\textbf{L}-\textbf{P}-E-S, la lettre \textbf{L} arrive avant la lettre \textbf{P}. On doit donc obligatoirement poser le logarithme comme la fonction à dériver ($u$).

\subsubsection*{Étape 2 : Mise en place de l'IPP}
On pose :
\begin{align*}
    u(x) &= \ln(x) \quad \implies \quad u'(x) = \frac{1}{x} \\
    v'(x) &= x \quad \implies \quad v(x) = \frac{x^2}{2}
\end{align*}
Les fonctions $u$, $u'$, $v$ et $v'$ sont continues sur l'intervalle $[1, e]$.

\subsubsection*{Étape 3 : Calcul de l'intégrale}
En appliquant la formule de l'IPP $\left(\int u v' = [uv] - \int u'v\right)$ :
\[ I = \left[ \ln(x) \cdot \frac{x^2}{2} \right]_{1}^{e} - \int_{1}^{e} \frac{1}{x} \cdot \frac{x^2}{2} \, dx \]

Simplifions le terme sous la deuxième intégrale $\left(\frac{1}{x} \cdot \frac{x^2}{2} = \frac{x}{2}\right)$ :
\[ I = \left( \ln(e) \cdot \frac{e^2}{2} - \ln(1) \cdot \frac{1^2}{2} \right) - \int_{1}^{e} \frac{x}{2} \, dx \]

Comme $\ln(e) = 1$ et $\ln(1) = 0$, on obtient :
\[ I = \frac{e^2}{2} - \left[ \frac{x^2}{4} \right]_{1}^{e} \]
\[ I = \frac{e^2}{2} - \left( \frac{e^2}{4} - \frac{1^4}{4} \right) \]
\[ I = \frac{2e^2}{4} - \frac{e^2}{4} + \frac{1}{4} \]
\[ I = \frac{e^2 + 1}{4} \]

\textbf{Conclusion :} Le calcul par IPP guidé par la méthode ALPES nous donne :
\[ \int_{1}^{e} x \ln(x) \, dx = \frac{e^2 + 1}{4} \]
