...
\definecolor{curvegreen}{RGB}{34, 177, 76}

\pgfplotsset{compat=1.18} % Indique à pgfplots d'utiliser les coordonnées standard
...
\end{enumerate}
\item \textcolor{blue}{\underline{Limites de fonctions trigonométriques}}      
       	\begin{itemize}
...
\]
    ...
    \item \textcolor{green}{\underline{\textbf{Asymptote verticale (AV)[Parallèle à \((oy)\)]}}} :
...
-\textbf{\underline{Continuité de \( f \):}}

La fonction \( f(x) = \frac{2x + 1}{x - 1} \) est une fonction rationnelle donc continue sur \( D_f = \mathbb{R} \setminus \{1\} \).

-\textbf{\underline{Monotonie de \( f \):}}
...
Pour tout \( x \in D_f \) \( f'(x) < 0 \).
  
Cela signifie que la fonction \( f \) est strictement décroissante sur chaque intervalle de son domaine de définition, à savoir \( ]-\infty; 1[ \) et \( ]1; +\infty[ \).
...
-\(
f(]1; +\infty[) = ]\lim_{x \to +\infty}f(x) ; \lim_{x \to 1^{+}}f(x)[ = ]2; +\infty[
\)			
			
\item \textbf{\textcolor{blue}{\underline{Théorème des valeurs intermédaires}}}
...
\textbf{\textcolor{blue}{\underline{Illustration graphique}}}
...
\(f( [-2 ; 4] ) = [\lim_{x \to -2} f(x) ; \lim_{x \to 4} f(x)] = [-11 ; 31] \).

D'après le théorème précédent, comme  \(5\in [-11 ; 31] \) donc l'équation \( f(x) = 5 \) a une solution dans \( [-2 ; 4] \).
...
Si la fonction \( f \) est \textcolor{red}{ continue } et \textcolor{red}{ strictement monotone } (croissante ou bien décroissante) sur \( [a ; b] \). Pour tout réel \( m \) compris entre \( f(a) \) et \( f(b) \), alors l'équation \( f(x) = m \) admet une \textcolor{red}{unique solution } dans \(\alpha\in [a ; b] \).
...
\begin{itemize}
    ...
    \item La monotonie de \( f^{-1} \) est de même sens que celle de \( f \) .
\end{itemize}
...
NB: Il n'est pas toujours possible de déterminer l'expression explicite de \( f^{-1} \)
...
\subsection*{\underline{\textbf{\textcolor{blue}{1.Notion de nombre dérivé}}}}

Soit \( f \) une fonction définie sur un intervalle \( I \) contenant \( x_{0} \)

\( f \) est dérivable en \( x_{0} \) ou \( f \) admet un nombre dérivé en \( x_{0} \) si et seulment si \[\lim_{x \to x_{0}}\frac{f(x)-f(x_{0})}{x-x_{0}}=\ell \text{ avec } (\ell \in\mathbb{R})\] \( \ell \) est appelé le nombre dérivé de \( f \) en \( x_{0} \) et est noté \( f'(x_{0}) \).
...
\subsection*{\underline{\textbf{\textcolor{blue}{2.Dérivabilté à gauche-Dérivabilité à droite}}}}
...
\( f(x) = x|x-1| \) \( f \) est-elle dérivable en \( 1 \) ?
...
Soit \( f \) une fonction dérivable sur un intervalle \( I \) et \( M\in\mathbb{R}^{*}_{+} \) tel que 
\( |f'(x)| \leq M \).

Alors pour tout \( a \) et \(  b \in I\), on a : \( | f(b)-f(a)  | \leq M(b-a)    \)


\textbf{\exemple}

Démontrer que pour tout réels \( a \) et \(  b \) \( | \cos b - \cos a  \leq | b - a |\)
...
\hline
\centering \textbf{Primitive de } \(u' u^n\) &
\[
\frac{u^{n+1}}{n+1}
\]
\\ \hline

\centering \textbf{Primitive de } \(\dfrac{u'}{u^n}\quad n\neq 1\) &
\[
-\frac{1}{(n-1)u^{\,n-1}}
\]
\\ \hline

\centering \textbf{Primitive de } \(\dfrac{u'}{\sqrt{u}}\) &
\[
2\sqrt{u}
\]
\\ \hline

\centering \textbf{Primitive de } \(f(ax+b)\) &
\[
\frac{1}{a} F(ax+b)
\]
\\ \hline
...
1.Déterminer toutes les primitives des fonctions suivantes, sur un intervalle bien choisi : 
$$\begin{array}{lll}
\displaystyle f_1(x)=5x^3-3x+7&\displaystyle f_2(x)=2\cos(x)-3\sin(x)&\displaystyle f_3(x)=\\
\displaystyle f_4(x)=\frac{5}{\sqrt x}+\frac{4}{x^4}+\frac{2}{x^2}+\frac{2}{x^3}&\displaystyle f_5(x)=\frac{x+5}{x^2}&\displaystyle f_6(x)=\frac{x^2}{5}+\frac 1{6}\\
\displaystyle f_3(x)=\sin(2x) & \displaystyle f_4(x)=\cos\left(3x+\frac\pi 3\right) & \displaystyle f_5(x)=(2x+1)^2\\
\displaystyle f_6(x)=\frac{3}{\sqrt{5x+1}} & \displaystyle f(x)=\frac{x}{1+x^2} &\displaystyle m(x)=3x\sqrt{1+x^2}\\
\end{array}$$
2.Reconnaissance de formes
$$\begin{array}{lll}
\mathbf 1.\ f(x)=(3x-1)(3x^2-2x+3)^3,\ I=\mathbb R&\quad&\mathbf 2.\ f(x)=\frac{1-x^2}{(x^3-3x+2)^3},\ I=]-\infty,-2[\\
\mathbf 3.\ f(x)=\frac{(x-1)}{\sqrt{x(x-2)}},\ I=]-\infty,0[&&\mathbf 4.\ f(x)=\frac{-2x}{(1+x^{2})^{2}},\ I=]1,+\infty[.
\end{array}$$
...
Soit $f$ une fonction et $D_f$ son ensemble de définition. La fonction $f$ est dite paire lorsque, pour tout $x$ appartenant à $D_f$, on a :
\[
-x \in D_f \text{ et } f(-x) = f(x).
\]

\textbf{NB} La courbe d'une fonction paire est symétrique par rapport à l'axe des ordonnées.
...
\subsection*{\underline{\textbf{\textcolor{blue}{3. Fonction impaire}}}}
...
\begin{tikzpicture}
    \begin{axis}[
        axis lines = center,
        xmin = -2.2, xmax = 4.5, % Élargi pour donner un maximum de place au texte de gauche
        ymin = -0.8, ymax = 4.5,
        ticks = none,
        enlargelimits = false,
        ...
        \draw[-{Stealth[scale=1.2]}, thick] (axis cs:0,0) -- (axis cs:0.6,0) node[below, midway] {$\vec{i}$};
        \draw[-{Stealth[scale=1.2]}, thick] (axis cs:0,0) -- (axis cs:0,0.6) node[left, midway] {$\vec{j}$};
        ...
        \addplot[
            ...
            color=blue,
            ultra thick
        ] {0.8*(x-2.2)^2 + 1} node[right, pos=0.95] {$C_f$};

        % Axe de symétrie x = a
        \draw[color=magenta, thick] (axis cs:2.2,-0.5) -- (axis cs:2.2,4.2);
        \node[color=magenta, below] at (axis cs:2.2,-0.5) {$x = a$};

        % Lignes de rappel en pointillés
        \draw[dashed, thick] (axis cs:1.1,0) -- (axis cs:1.1,1.968) -- (axis cs:0,1.968);
        \draw[dashed, thick] (axis cs:3.3,0) -- (axis cs:3.3,1.968) -- (axis cs:0,1.968);

        % Étiquettes sur l'axe x
        \node[below] at (axis cs:1.1,0) {$a - h$};
        \node[below] at (axis cs:3.3,0) {$a + h$};

        % Étiquette sur l'axe y (Corrigée pour éviter le bug d'alignement vertical)
        \node[left, text width=2cm, align=right] at (axis cs:-0.1,1.968) {$f(a+h)$ \\ $=$ \\ $f(a-h)$};
...
\begin{tikzpicture}
    \begin{axis}[
        ...
        % ----------------------------------------------------
        % GEOMETRIE ACCENTUEE :
        % f(x) = (x-2.5)^3 - 0.5*(x-2.5) + 2
        % Centre \Omega : a = 2.5, b = 2
        % Pour h = 1.2 :
        %   a - h = 1.3  => f(1.3) = 0.872
        %   a + h = 3.7  => f(3.7) = 3.128
        % ----------------------------------------------------

        % 1. Tracé de la courbe C_f avec plus de pente
        \addplot[
            domain=0.8:4.1,
            samples=100,
            color=curvegreen,
            thick
        ] {(x-2.5)^3 - 0.5*(x-2.5) + 2} node[right, pos=0.95, color=curvegreen] {$C_f$};

        % 2. Segment oblique bien pentu passant par \Omega
        \draw[thick, gray!80!black] (axis cs:1.3, 0.872) -- (axis cs:3.7, 3.128);

        % 3. Lignes de rappel pour le centre \Omega (en rouge)
        \draw[red, dashed, thick] (axis cs:2.5, 0) -- (axis cs:2.5, 2);
        \draw[red, dashed, thick] (axis cs:0, 2) -- (axis cs:2.5, 2);

        % 4. Lignes de rappel pour a-h et a+h (bien séparées verticalement)
        \draw[dashed] (axis cs:1.3, 0) -- (axis cs:1.3, 0.872) -- (axis cs:0, 0.872);
        \draw[dashed] (axis cs:3.7, 0) -- (axis cs:3.7, 3.128) -- (axis cs:0, 3.128);

        % 5. Marquage des 3 points par des croix 'x'
        \node[mark=x, mark size=4pt, thick] at (axis cs:1.3, 0.872) {};
        \node[mark=x, mark size=4pt, thick] at (axis cs:2.5, 2) {};
        \node[mark=x, mark size=4pt, thick] at (axis cs:3.7, 3.128) {};

        % 6. Placements des étiquettes (bien aérées sur l'axe y)
        % Axe des abscisses (x)
        \node[below] at (axis cs:1.3, 0) {$a - h$};
        \node[below, red] at (axis cs:2.5, 0) {$a$};
        \node[below] at (axis cs:3.7, 0) {$a + h$};

        % Axe des ordonnées (y)
        \node[left, xshift=-2pt] at (axis cs:0, 0.872) {$f(a - h)$};
        \node[left, red, xshift=-2pt] at (axis cs:0, 2) {$b$};
        \node[left, xshift=-2pt] at (axis cs:0, 3.128) {$f(a + h)$};

        % Nom du centre \Omega
        \node[red, above left] at (axis cs:2.5, 2) {\Large $\Omega$};

    \end{axis}

    % Légende sous le graphique
    \node[red, below=0.4cm] at (current bounding box.south) {$\Omega(a, b)$ centre de symétrie};
\end{tikzpicture}
...
\begin{figure}[H]% Forcer l'image à cet endroit
\centering
\includegraphics[width=0.5\textwidth]{centreSymetrie.png}
\caption{centreSymetrie}
\label{fig:monimage}
\end{figure}
\end{comment}

\textbf{\textcolor{blue}{Cas particulier}} Si \( a=b=0 \) ; \( f \) est une fonction impaire

\textbf{\underline{\exemple}}

\( f(x)=\frac{3x+1}{2x-6} \) Montrer que \( I(3 ; \frac{3}{2}) \) est centre de symétrie pour la courbe \( C_{f} \).
...
$T$ est une période de $f$ lorsque, pour tout $x$ appartenant à $D_f$, on a :
...
Considérons la fonction $f : \mathbb{R} \rightarrow \mathbb{R}$ définie par $f(x) = \sin(x)$ pour tout $x \in \mathbb{R}$.
...
% Tracer la courbe pour x <= 0
\addplot[blue, thick] {-x + sqrt(x^2 - 2*x)};
\addlegendentry{Pour \( x \leq 0 \)};

% Tracer la courbe pour x > 0
\addplot[red, thick] {2*x*sqrt(1 - x^2)};
\addlegendentry{Pour \( x > 0 \)};
