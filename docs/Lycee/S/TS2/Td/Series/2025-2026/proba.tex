...
% ==========================================
\usepackage[left=1cm, right=1cm, top=1cm, bottom=1.7cm]{geometry} % Définition des marges du document
...
\renewcommand{\headrulewidth}{0pt}  % Supprime le trait de séparation en haut de page
\renewcommand{\footrulewidth}{1pt}  % Trait horizontal au-dessus du pied de page
\renewcommand{\footruleskip}{10pt}  % Espacement entre le texte et le trait du pied de page
...
% 3. MATHÉMATIQUES
% ==========================================
\usepackage{amsmath}     % Environnements et commandes mathématiques de base (équations, matrices, etc.)
\usepackage{amssymb}     % Extension des symboles mathématiques usuels (ex: \mathbb{R})
\usepackage{mathrsfs}    % Ajoute la police calligraphique mathématique (\mathscr)

% Forcer l'affichage grand format des symboles (fractions, sommes, limites) même dans le texte
\everymath{\displaystyle}

% ==========================================
% 4. TABLEAUX ET MISE EN PAGE DES OBJETS
% ==========================================
\usepackage{array}       % Améliore la gestion et l'alignement des colonnes de tableaux
\usepackage{multirow}    % Permet de fusionner plusieurs lignes dans un tableau
\usepackage{varwidth}    % Permet de créer des boîtes adaptatives en largeur
\usepackage{float}       % Fournit l'option [H] pour forcer le placement des objets flottants (images/tableaux)
...
\usetikzlibrary{trees, patterns} % Extensions TikZ :
                                 % - trees    : pour réaliser des arbres de dénombrement ou probabilités
                                 % - patterns : pour le remplissage avec des motifs (hachures, etc.)

\usepackage{tkz-tab}     % Paquet spécifique pour tracer des tableaux de variations et de signes

% ==========================================
% 6. LISTES ET PUCES PERSONNALISÉES
% ==========================================
\usepackage{enumitem}    % Permet de personnaliser l'apparence des listes à puces et numérotées

% Macro pour créer une puce numérotée dans un cercle rouge avec TikZ
\newcommand\circitem[1]{%
\tikz[baseline=(char.base)]{
\node[circle, draw=gray, fill=red!55, minimum size=1.2em, inner sep=0] (char) {#1};}}

% Macro pour créer une puce alphabétique dans un carré cyan avec TikZ
\newcommand\boxitem[1]{%
\tikz[baseline=(char.base)]{
\node[fill=cyan, minimum size=1.2em, inner sep=0] (char) {#1};}}

% Configuration du style de puces pour le niveau 1 des listes d'énumération
\setlist[enumerate,1]{label=\protect\circitem{\arabic*}}

% ==========================================
% 7. BOÎTES ET ENCADRÉS (TCOLORBOX)
% ==========================================
\usepackage[most]{tcolorbox} % Moteur complet pour fabriquer des encadrés stylisés
\tcbuselibrary{skins, hooks} % Extensions de tcolorbox :
                             % - skins : styles graphiques avancés
                             % - hooks : gestion fine du comportement interne de la boîte

% Définition de l'environnement d'encadré personnalisé "exa" pour les exemples/exercices
...
% ==========================================
\usepackage{eso-pic}     % Permet de superposer des éléments (images, texte) en arrière-plan des pages

% Ajout du filigrane "PGB" en gris au centre de toutes les pages du document
\AddToShipoutPicture{
    \AtTextCenter{%
...
% ==========================================
% --- Compteur pour les exercices ---
\newcounter{exercice}                                   % Création du compteur d'exercices
\renewcommand{\theexercice}{\arabic{exercice}}         % Format d'affichage en chiffres arabes
\newcommand{\exo}{\refstepcounter{exercice}\textbf{Exercice \theexercice}\ } % Macro \exo pour affichage

% --- Compteur pour la correction des exercices ---
\newcounter{correction}                                 % Création du compteur de corrections
\renewcommand{\thecorrection}{\arabic{correction}}     % Format d'affichage en chiffres arabes
\newcommand{\correc}{\refstepcounter{correction}\textbf{Correction \thecorrection}\ } % Macro \correc pour affichage

% ==========================================
% 10. HYPERLIENS ET INTERACTIVITÉ
% ==========================================
% Charge hyperref en TOUT DERNIER pour éviter les conflits de redéfinition avec d'autres packages
...
Un jeu réunit 35 hommes, 40 femmes et 25 enfants. Sur une table, il y a 3 urnes $U_1$, $U_2$ et $U_3$ contenant des boules indiscernables au toucher. L'urne $U_1$ contient 1 boule noire, 5 boules blanches et 4 boules rouges ; l’urne $U_2$ contient 4 boules noires, 5 boules blanches et 1 boule rouge ; l’urne $U_3$ contient 8 boules noires et 2 boules rouges.
...
\section*{\fbox{\textbf{\correc}}}
% Styles pour les différents niveaux de l'arbre
\tikzstyle{level 1}=[level distance=4cm, sibling distance=4.5cm] % Niveau 1 : distance verticale et entre branches
\tikzstyle{level 2}=[level distance=5cm, sibling distance=2cm] % Niveau 2 : plus resserré
...
\node[bag] {\textcolor{darkred}{}} 
    child {
        ...
        node[bag] {\textcolor{darkred}{$E$}} 
            child {
                % Sous-branche de B : issue V
                node[end] {\textcolor{darkred}{$R$}} 
                edge from parent
                node[above] {\textcolor{black}{$\frac{2}{10}$}} % Probabilité conditionnelle : P(V|B)
            }
            child {
                % Sous-branche de B : issue R
                node[end] {\textcolor{darkred}{$N$}} 
                edge from parent
                node[above] {\textcolor{black}{$\frac{8}{10}$}} % Probabilité à compléter
            }
        edge from parent
        node[above] {\textcolor{black}{$\frac{25}{100}$}} % Probabilité de B : P(B)
    }
        child {
        % Première branche : événement B
        node[bag] {\textcolor{darkred}{$F$}} 
            child {
                % Sous-branche de B : issue V
                node[end] {\textcolor{darkred}{$R$}} 
                edge from parent
                node[above] {\textcolor{black}{$\frac{1}{10}$}} % Probabilité conditionnelle : P(V|B)
            }
            child {
                % Sous-branche de B : issue M
                node[end] {\textcolor{darkred}{$B$}} 
                edge from parent
                node[above] {\textcolor{black}{$\frac{5}{10}$}} % Probabilité à compléter
            }
            child {
                % Sous-branche de B : issue R
                node[end] {\textcolor{darkred}{$N$}} 
                edge from parent
                node[above] {\textcolor{black}{$\frac{4}{10}$}} % Probabilité à compléter
            }
        edge from parent
        node[above] {\textcolor{black}{$\frac{40}{100}$}} % Probabilité de B : P(B)
    }
    child {
        % Deuxième branche : événement A
        node[bag] {\textcolor{darkred}{$H$}} 
            child {
            ...
            child {
                ...
                node[end] {\textcolor{darkred}{$N$}} 
                edge from parent
                node[above] {\textcolor{black}{$\frac{1}{10}$}} % Probabilité conditionnelle : P(R|A)
            }
        edge from parent
        node[above] {\textcolor{black}{$\frac{35}{100}$}} % Probabilité de A : P(A), à compléter
    };
...
\section*{\fbox{\textbf{Exercice 17}}\quad BAC S2 2022}
...
\begin{enumerate}
    ...
    \end{enumerate}
\end{enumerate}
\section*{\fbox{\textbf{Correction 17}}\quad BAC S2 2022}
\end{document}