...
\section*{\underline{\textbf{\textcolor{darkred}{II. Notion d'application}}}}
...
\underline{\textbf{\textcolor{darkred}{1\textsuperscript{ère} manière}}}
...
Les réels supérieurs à $-1$ ont $2$ antécédents par $f$ dans
$\mathbb{R}$.

Donc $f$ n'est pas une injection.


\underline{\textbf{\textcolor{darkred}{Autrement :}}}
...
\subsection*{\underline{\textbf{\textcolor{darkgreen}{4) Application surjective ou surjection}}}}

\subsubsection*{\underline{\textbf{a) Définition :}}}

Une application $f$ définie de $A$ vers $B$ est dite
\textcolor{darkred}{\textbf{surjective}} (on dit que $f$ est une surjection)
si chaque élément de $B$ est l'image au moins par $f$ d'un élément de $A$.
...
Pour montrer que l'application $f$ n'est pas surjective, il suffit de trouver
un $y\in B$ qui n'a pas d'antécédent dans $A$.
...
Donc $f$ est une \textcolor{darkred}{surjection}.
...
\begin{tikzpicture}[scale=0.85, every node/.style={scale=0.9}]
    ...
    % Ensembles D et C
    \node[text=darkred, font=\bfseries] at (0,2.3) {D};
    \node[text=darkred, font=\bfseries] at (2.5,2.3) {C};
...
Par conséquent, $g$ n'est pas une \textcolor{darkred}{surjection}.

\subsubsection*{\underline{\textbf{\textcolor{darkred}{Exemple :}}}}
...
\section*{\underline{\textbf{\textcolor{darkred}{III. Composée de fonctions}}}}

\subsection*{\underline{\textbf{\textcolor{darkgreen}{1) Définition}}}}

Soient $E$, $F$, $G$ 3 parties non vides de $\mathbb{R}$,
$f:E\longrightarrow F$ et $g:F\longrightarrow G$,
avec domaine de définition $D_f$ et $g$ avec domaine de définition $D_g$.

On appelle \textcolor{darkred}{\textbf{composée de $f$ suivie de $g$}},
la fonction qui à $x$ associe $g(f(x))$ notée
\textcolor{darkred}{\textbf{$g\circ f$}} (on lit « $g$ rond $f$ »).
...
\subsection*{\underline{\textbf{\textcolor{darkgreen}{2) Domaine de définition}}}}

La composée $g\circ f(x)$ n'a de sens que si $x\in D_f$ et
$f(x)\in D_g$. De même que $f\circ g(x)$ n'a de sens que si
$x\in D_g$ et $g(x)\in D_f$.
...
\subsubsection*{\textcolor{darkred}{Exemple :}}
...
\begin{enumerate}
    \item Déterminer $D_f$, $D_g$, $D_{f\circ g}$, $D_{g\circ f}$.
    
    \item Exprimer $g\circ f(x)$.
    
    \item Calculer l'image des réels suivants par $f\circ g$ si possible :
    $0$ et $2$.
\end{enumerate}
...
\[
\begin{aligned}
*\,D_g &: \quad g\text{ existe si }x+1\geq0\\
&\quad\Longrightarrow x\geq-1
\end{aligned}
\]
...
\{x\in[-1\,;\,+\infty[
\mid
\sqrt{x+1}\in\mathbb{R}\setminus\{1\}\}\\
&=
...
D_{g\circ f}
&=
\{x\in\mathbb{R}\setminus\{1\}\mid f(x)\in[-1\,;\,+\infty[\}\\
&=
\left\{x\neq1\mid
\frac{2x-5}{x-1}\geq-1\right\}
\end{aligned}
\]
...
\[\textbf{Or }
\begin{aligned}
\dfrac{2x-5}{x-1}\geq-1
&\Longrightarrow
\dfrac{2x-5}{x-1}+1\geq0\\
&\Longrightarrow
\dfrac{3x-6}{x-1}\geq0
\end{aligned}
\]

Posons $3x-6=0\Longrightarrow x=2$
et $x\neq1$.
...
\[
\text{Ce qui donne }x\in]-\infty\,;\,1[\cup[2\,;\,+\infty[
\]
...
\textbf{3) Calculons l'image de $0$ et $2$}
...
g(2)&=\sqrt{3}
\Longrightarrow
f\circ g(2)=\frac{2\sqrt{3}-5}{\sqrt{3}-1}\\
f\circ g(2)
&=\frac{(2\sqrt{3}-5)(\sqrt{3}+1)}
{3-1}
\end{aligned}
\]
...
\section*{\underline{\textbf{\textcolor{darkred}{IV. Image directe -- Image réciproque par une application}}}}

\subsection*{\underline{\textbf{\textcolor{darkgreen}{1) Image directe}}}}

\subsubsection*{\underline{\textbf{a) Définition :}}}

Soit $f$ une fonction définie de $E\longrightarrow F$, d'ensemble de définition
$D_f$ et $A$ une partie de $E$.

On appelle \textcolor{darkred}{\textbf{image directe}} (image de $A$ par $f$)
notée $f(A)$, l'ensemble des images par $f$ de tous les éléments de $D_f \cap A$.
...
\begin{aligned}
x\in A
&\Longrightarrow -1<x\leq4\\
&\Longrightarrow 0<x+1\leq5\\
&\Longrightarrow \frac{1}{5}\leq\frac{1}{x+1}\\
&\Longrightarrow \frac{1}{5}\leq f(x)
\end{aligned}
\]
...
\begin{aligned}
x\in[-5\,;\,-1[
&\quad\text{ou}\quad x\in]-1\,;\,3]\\
-5\leq x<-1
&\quad\text{ou}\quad -1<x\leq3\\
-4\leq x+1<0
&\quad\text{ou}\quad 0<x+1\leq4\\
-\frac{1}{4}\geq\frac{1}{x+1}
&\quad\text{ou}\quad \frac{1}{4}\leq\frac{1}{x+1}\\
f(x)\leq-\frac{1}{4}
&\quad\text{ou}\quad f(x)\geq\frac{1}{4}
\end{aligned}
\]
...
\subsection*{\underline{\textbf{\textcolor{darkgreen}{2) Image réciproque}}}}

\subsubsection*{\underline{\textbf{a) Définition :}}}

Soit $f$ une fonction définie de $E$ vers $F$, d'ensemble de définition
$D_f$ et $B$ une partie de $F$.

On appelle \textcolor{darkred}{\textbf{image réciproque de $B$ par $f$}},
notée $f^{-1}(B)$, l'ensemble des antécédents par $f$ de tous les éléments de $B$.

\subsubsection*{\textbf{b) Remarque : Comment trouver l'image réciproque ?}}

Pour trouver l'image réciproque d'un intervalle $B$ dans $\mathbb{R}$ par une
fonction $f$ d'ensemble de définition $D_f$, on résout par des systèmes
d'inconnues suivants :

\begin{itemize}

    \item Si $B=[a\,;\,b]$, alors on résout :
    \(
    \begin{cases}
        x\in D_f\\
        a\leq f(x)\leq b
    ...
    \item Si $B=]-\infty\,;\,b]$, alors on résout :
    \(
    \begin{cases}
        x\in D_f\\
        f(x)\leq b
    ...
    \item Si $B=[a\,;\,+\infty[$, alors on résout :
    \(
    \begin{cases}
        x\in D_f\\
        f(x)\geq a
...
\subsubsection*{\underline{\textbf{c) Exemple :}}}
...
\textbf{1) Pour $B=[1\,;\,3]$, déterminons l'image réciproque par $f$ :}
...
\subsubsection*{\underline{\textbf{\textcolor{darkred}{REMARQUE !!!}}}}

Toute application à la fois \textcolor{darkred}{injective} et
\textcolor{darkred}{surjective} est forcément \textcolor{darkred}{bijective}.
\end{document}
