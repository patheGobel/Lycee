...
\section*{\color{myred}III. Continuité des fonctions}
...
\paragraph{Exemple :}
La fonction \( f(x) = x^2 \) est continue en tout réel \( a \) car :
\[
\lim_{x \to a} x^2 = a^2 = f(a)
\]

\subsection*{\color{myred}2. Continuité sur un intervalle}

On dit que \( f \) est \textbf{continue sur un intervalle \( I \)} si elle est continue en tout point de \( I \).

\paragraph{Exemples :}
\begin{itemize}
    \item Les fonctions polynômes sont continues sur \( \mathbb{R} \).
    \item Les fonctions rationnelles (quotient de deux polynômes) sont continues sur chaque intervalle où le dénominateur ne s'annule pas.
    \item La fonction racine carrée \( x \mapsto \sqrt{x} \) est continue sur \( [0; +\infty[ \).
    \item La fonction valeur absolue \( x \mapsto |x| \) est continue sur \( \mathbb{R} \).
\end{itemize}

\subsection*{\color{myred}3. Opérations sur les fonctions continues}

\begin{enumerate}
    \item La \textbf{somme} de deux fonctions continues est continue.
    \item Le \textbf{produit} de deux fonctions continues est continue.
    \item Le \textbf{quotient} de deux fonctions continues est continue (là où le dénominateur ne s'annule pas).
    \item La \textbf{composée} de deux fonctions continues est continue.
\end{enumerate}

\paragraph{Exemple :}
La fonction \( f(x) = \frac{2x^2 - 3x + 1}{x - 5} \) est continue sur \( ]-\infty; 5[ \) et sur \( ]5; +\infty[ \) car c'est un quotient de deux polynômes, et le dénominateur \( x - 5 \) s'annule en 5.

\subsection*{\color{myred}4. Continuité à gauche et à droite}

\begin{itemize}
    \item \( f \) est \textbf{continue à gauche} en \( a \) si :
    \[
    \lim_{x \to a^-} f(x) = f(a)
    \]
    \item \( f \) est \textbf{continue à droite} en \( a \) si :
    \[
    \lim_{x \to a^+} f(x) = f(a)
    \]
\end{itemize}

La fonction est continue en \( a \) si et seulement si elle est continue à gauche \textbf{et} à droite en \( a \).
...
\]
\begin{itemize}
    \item À gauche en 0 : \( \lim\limits_{x \to 0^-} f(x) = \lim\limits_{x \to 0^-} (x+1) = 1 \)
    \item À droite en 0 : \( \lim\limits_{x \to 0^+} f(x) = \lim\limits_{x \to 0^+} (x^2+1) = 1 \)
    \item Mais \( f(0) = 2 \neq 1 \)
\end{itemize}
Donc \( f \) n'est pas continue en 0 (les limites à gauche et à droite existent et sont égales, mais elles ne valent pas \( f(0) \)).

\subsection*{\color{myred}5. Prolongement par continuité}

Soit \( f \) une fonction définie sur \( I \setminus \{a\} \). Si \( \lim\limits_{x \to a} f(x) = L \) (limite finie), alors on peut \textbf{prolonger} \( f \) par continuité en \( a \) en posant :
\[
f(a) = L
\]
...
On prolonge \( f \) par continuité en 1 en posant :
\[
f(1) = 2
\]

Ainsi, la nouvelle fonction est continue sur \( \mathbb{R} \).

\subsection*{\color{myred}6. Continuité des fonctions usuelles}

\begin{tcolorbox}[colback=gray!5!white, colframe=black!75!black]
\begin{itemize}
    \item Toute fonction polynôme est continue sur \( \mathbb{R} \).
    \item Toute fonction rationnelle est continue sur tout intervalle où son dénominateur ne s'annule pas.
    \item La fonction racine carrée \( x \mapsto \sqrt{x} \) est continue sur \( [0; +\infty[ \).
    \item La fonction valeur absolue \( x \mapsto |x| \) est continue sur \( \mathbb{R} \).
    \item La fonction inverse \( x \mapsto \frac{1}{x} \) est continue sur \( ]-\infty; 0[ \) et sur \( ]0; +\infty[ \).
\end{itemize}
\end{tcolorbox}

\subsection*{\color{myred}7. Synthèse}

\begin{tcolorbox}[colback=gray!5!white, colframe=black!75!black]
\begin{itemize}
    \item Une fonction est continue si on peut tracer sa courbe "sans lever le crayon".
    \item Les fonctions polynômes, rationnelles (sur leur domaine), racine carrée et valeur absolue sont continues.
    \item On peut prolonger une fonction par continuité lorsque la limite existe et est finie.
    \item Pour vérifier la continuité d'une fonction définie par morceaux, on étudie les limites à gauche et à droite.
\end{itemize}
\end{tcolorbox}

\paragraph{Exercices d'application :}

\begin{enumerate}
    \item Soit \( f(x) = \frac{x^2 - 4}{x - 2} \). Déterminer le domaine de définition de \( f \), puis prolonger \( f \) par continuité si possible.
    ...
    \]
    \begin{enumerate}
        \item Calculer \( \lim\limits_{x \to 1^-} f(x) \) et \( \lim\limits_{x \to 1^+} f(x) \).
        \item La fonction est-elle continue en 1 ? Justifier.
    \end{enumerate}

    \item Étudier la continuité de la fonction \( f \) définie par :
    \[
    f(x) = 
    \begin{cases}
    ...
    \item Soit \( f(x) = \frac{2x^2 - 3x + 1}{x^2 - 4} \). Sur quels intervalles \( f \) est-elle continue ? Justifier.
\end{enumerate}
\end{document}